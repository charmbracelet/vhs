...
func TestParseSource(t *testing.T) {
	...
	t.Run("should return errors when tape exits but is empty", func(t *testing.T) {
		test := &parseSourceTest{
			tape:      "Source source.tape",
			errors:    []string{"Source tape: source.tape is empty"},
			writeFile: true,
		}
	...
	t.Run("should return errors when tape NOT found", func(t *testing.T) {
		test := &parseSourceTest{
			tape:      "Source source.tape",
			errors:    []string{"Source tape source.tape not found"},
			writeFile: false,
		}
	...
	t.Run("should return error when tape extension is NOT (.tape)", func(t *testing.T) {
		test := &parseSourceTest{
			...
			errors:    []string{"Expected a tape file with .tape extension"},
			writeFile: true,
		}
	...
	t.Run("should return error when Source command does NOT have tape path", func(t *testing.T) {
		test := &parseSourceTest{
			tape:      "Source",
			errors:    []string{"Expected tape path after source"},
			writeFile: true,
		}
	...
	Type "goodbye"
	`,
			errors:    []string{"Nested source is not allowed"},
			writeFile: true,
		}
