...
func Evaluate(ctx context.Context, tape string, out io.Writer, opts ...EvaluatorOption) []error {
	...
	for _, cmd := range cmds {
		if cmd.Type == token.SET && (cmd.Options == shellSetting || cmd.Options == promptColorSetting) || cmd.Type == token.ENV {
			err := Execute(cmd, &v)
			if err != nil {
				return []error{err}
			}
	...
	for i, cmd := range cmds {
		if cmd.Type == token.SET || cmd.Type == token.OUTPUT || cmd.Type == token.REQUIRE {
			...
			if cmd.Options != shellSetting && cmd.Options != promptColorSetting {
				err := Execute(cmd, &v)
				if err != nil {
					return []error{err}
				}
